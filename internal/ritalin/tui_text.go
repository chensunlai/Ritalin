...
var englishUI = map[string]string{
	...
	"选择使用节点…":                   "Choose proxy node…",
	"默认连接":                      "Default connection",
	"使用连接":                      "Connection",
	"请选择节点":                     "Select a node",
	"选择节点":                      "Choose a node",
	"← 返回":                      "← Back",
	"默认连接使用当前网络设置；代理节点使用为状态指定的节点。": "Default uses your current network settings; Proxy node uses the node assigned to the state.",
	"选择检测通过的节点，Esc 返回。":            "Choose a checked node. Esc to go back.",
	"请先在代理页添加并检测节点":                "Add and check nodes on the Proxies page first.",
	"请在使用页为此状态选择检测通过的节点":           "Choose a checked node for this state on the Use page.",
	"节点没有可用的代理地址":                  "The node has no proxy address.",
	"发送（替换后）":                      "Sending (after replacement)",
	"接收（服务器原始，未替换）":                "Received (original server header, before replacement)",
	"未收到 HTTP 响应":                  "No HTTP response received",
	"无":                            "Absent",
	"%d 字符":                        "%d characters",
	"密文 %d 字节 · %d 块":              "Ciphertext %d bytes · %d blocks",
	"布局无法解析":                       "Unrecognized layout",
	"已取消":                          "Cancelled",
	...
	"检测 %d 个节点（最多 4 个并行）": "Checking %d nodes (up to 4 at a time)",
	"[%d/%d] 已检测 %s":      "[%d/%d] Checked %s", "未保存：": "Not saved: ", "出口 IP：": "Exit IP: ",
	...
	"导入…": "Import…", "导出…": "Export…", "导入状态": "Imported state",
	"导入状态 JSON 文件路径":                "State JSON file path",
	"导出路径（文件包含完整状态）":                "Export path (file contains full turn states)",
	"从 JSON 文件导入到当前列表，自动跳过重复状态。":    "Import a JSON file into this list, skipping duplicates.",
	"将当前列表导出为 JSON；不包含凭证、HTML 或图片。": "Export this list as JSON, without credentials, HTML, or images.",
	"已导入 %d 个状态，跳过 %d 个重复状态":        "Imported %d states; skipped %d duplicates",
	"已导出 %d 个状态：%s":                 "Exported %d states: %s",
	"当前列表为空":                        "This list is empty", "状态文件不能超过 8 MiB": "State files must not exceed 8 MiB",
	"文件已存在，请换一个路径": "File already exists; choose a different path",
	"请选择 JSON 文件":  "Choose a JSON file", "无效的 Ritalin 状态文件": "Invalid Ritalin state file",
	"第 %d 个状态无效，未导入": "State %d is invalid; nothing was imported",
	"清空全部节点…":        "Clear all nodes…", "清空待确认状态…": "Clear pending states…", "清空可用状态…": "Clear usable states…",
	"确认清空此列表？HTML、图片和实验记录保留。": "Clear this list? HTML, images, and experiment logs will be kept.",
	"清空当前列表，保留 HTML、图片和实验记录。": "Clear this list, keeping HTML, images, and experiment logs.",
	"高级设置 →": "Advanced →", "← 返回设置": "← Back to settings", "d 删除": "d to delete",
	"Clash 节点": "Clash", "鹈鹕测试": "Pelican", "可用状态": "States", "设置": "Settings",
	...
	"导入节点后即可开始探测": "Import nodes to start probing", "添加状态，或从探测结果中选择": "Add a state or collect one with a probe",
	...
	"编辑": "Edit", "确认": "Confirm", "保存": "Save", "取消": "Cancel", "在此输入…": "Type or paste here…",
	...
	"审核结果":                  "Review result", "打开 HTML 后选择：": "Open the HTML, then choose:",
	"[p] 生成图片预览（可选）": "[p] Generate image preview (optional)",
	"正在生成预览…":        "Generating preview…", "预览失败，不影响审核": "Preview failed; review is still available",
	"Enter 审核":                                       "Enter to review",
	"[g] 保留   [b] 删除   [s] 稍后":                       "[g] Keep   [b] Delete   [s] Later",
	"Enter 保存 · Tab 切换 · Esc 取消":                     "Enter save · Tab focus · Esc cancel",
	"Enter 确认 · Tab 切换 · Esc 取消":                     "Enter confirm · Tab focus · Esc cancel",
	"Enter 换行 · Tab 选择保存 · Esc 取消":                   "Enter newline · Tab to Save · Esc cancel",
	"Esc 取消 · PgUp/PgDn 滚动 · End 跟随":                 "Esc cancel · PgUp/PgDn scroll · End follow",
	...
	"逐个生成动画，完成后由你选择保留或删除。":                   "Generate an animation for each state, then keep or delete the result.",
